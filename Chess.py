...
class Game:
    ...
    def __init__(self):
        ...
        self.turn = 'w'
        self.flag = {'w' :[True, True], 'b':[True, True]} # in order to do king-rook casting, if rook or king is moved then it becomes False and option isn't available anymore
...
def getBoard():
    ...
    if not valid:
        return g
    return "".join(["00" if not piece or piece is None else piece.name for I in matrixflip(g.board,'v') for piece in I])
...
def options():
    ...
    if CP.name[0] in ['K','R']:
        if g.flag[g.turn][0] == False and g.board[side][1] == g.board[side][2] == g.board[side][3] == False and g.board[side][0] != False and g.board[side][4].name[0] != False and g.board[side][0].name[0] == "R" and g.board[side][4].name[0] == "K":
            g.casting[g.turn][0] = True
            options.append(("c" if CP.name[0] == "K" else "d") +str(side+1))
        if g.flag[g.turn][1] == False and g.board[side][6] == g.board[side][5]  == False and g.board[side][0] != False and g.board[side][4].name[0] != False and g.board[side][7].name[0] == "R" and g.board[side][4].name[0] == "K" :
